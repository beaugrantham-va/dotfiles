...
local widget_clock = wibox.widget.textclock()
local widget_calendar = widget_calendar({ placement = 'top_right' })
widget_clock:connect_signal("button::press", function(_, _, _, button)
  if button == 1 then widget_calendar.toggle() end
end)
...
for i = 1, 10 do
  globalkeys = gears.table.join(
    globalkeys,
    -- View tag only.
    awful.key({ modkey }, "#" .. (i + 9 % 10), function ()
    ...
    -- Toggle tag display.
    awful.key({ modkey, "Control" }, "#" .. (i + 9 % 10), function ()
                  local screen = awful.screen.focused()
                  local tag = screen.tags[i]
                  if tag then
    ...
    -- Move client to tag.
    awful.key({ modkey, "Shift" }, "#" .. (i + 9 % 10), function ()
                  if client.focus then
    ...
    awful.key({ modkey, "Control", "Shift" }, "#" .. (i + 9 % 10), function ()
                  if client.focus then
                    local tag = client.focus.screen.tags[i]
                    if tag then
